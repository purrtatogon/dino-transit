...
// Metro Lisboa API: I only get waits + line status, not GPS per train. I fake lat/lng on the line graph
// so sprites move, but it's bumpier than the simulator. If this returns nothing, fallbacks take over.
@Service
...
// Which way the train is headed on the line (needed for forward vs backward on my ordered stop list).
                ...
                // Destination gives a coarse direction first; interpolation overwrites when it can.
                ...
                // Stable dinoName per train so React doesn't remount sprites when ML's ranking order shakes.
                // (I tried a rank index before — it flipped every refresh and wrecked continuity.)
    ...
    // Ordered station chains for each line (start → end).
    // I treat green/yellow as mostly north-south ("forward" toward south-ish), blue/red east-west-ish.
    ...
    /** Rough seconds I assume between neighbouring stops — tuning knob, not gospel. */
    private static final double INTER_STATION_SECONDS = 90.0;

    /** Below this wait time I snap the train to the platform coords (basically arrived). */
    ...
    // After interpolation: coords + compass dir that roughly matches how the drawn segment aims on the map.
    private record InterpolationResult(double lat, double lng, String geometryDirection) {}

    /**
     * Fake a lat/lng between stops from Metro's seconds-to-arrival hints.
     * Returns null when I can't line up names with my topology.
     */
    private InterpolationResult interpolateTrainPosition(
        ...
        // Parked — use the station coordinate
        ...
        // Step backwards toward the upstream stop from the ETA math
        ...
        // Aim the sprite arrow from segment geometry vs the messy whole-line heuristic
    ...
    /** Pick N/E/S/W from a topology hop; fallback if both stops sit on basically the same dot. */
    ...
    /** Fuzzy station name locate in my ordered arrays (API strings are messy). */
    ...
    /** Coordinate lookup for ML station payloads after I normalize spelling a bit. */
        ...
        for (String key : keys) {
            ...
            if (child != null && !child.isNull()) {
                ...
                try {
                    ...
                    // didn't parse, try another pattern
                }
            }
        }
        return null;
